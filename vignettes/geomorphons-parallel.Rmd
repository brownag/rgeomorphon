...
Regardless of which of the backends you configure with `future::plan()`, your call to `geomorphons()` remains identical. You write your code once, and can scale it up simply by changing the plan.
...
To force tiled processing we can manipulate the environment variables that `rgeomorphon` uses to estimate memory needs. We will set `R_RGEOMORPHON_MEM_SCALE_NEED` to tell the function how many parallel jobs we intend to run.
